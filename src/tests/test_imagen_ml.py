...
# Ejemplo de HTML que no muestra la imagen correctamente
html_sin_imagen = """
...
# Ejemplo de HTML que sí muestra la imagen correctamente
html_con_imagen = """
...
def generar_html_prueba(urls_imagenes):
    ...
    temp_file = os.path.join(os.getcwd(), "test_imagenes_ml.html")
...
def main():
    ...
    # Encontrar la variante que funciona (si existe)
    variante_funcional = next((v for v in variantes_sin_imagen if verificar_url_imagen(v)), None)
    
    if variante_funcional:
        print(f"Se encontró una variante que funciona: {variante_funcional}")
        
        # Analizar qué transformación funcionó
        if 'D_NQ_NP' in variante_funcional and 'D_Q_NP' in url_sin_imagen:
            print("- Reemplazar 'D_Q_NP' por 'D_NQ_NP'")
        
        if '-F.webp' in variante_funcional and '-V.webp' in url_sin_imagen:
            print("- Reemplazar '-V.webp' por '-F.webp'")
        
        if '.jpg' in variante_funcional and '.webp' in url_sin_imagen:
            print("- Probar cambiar el formato de '.webp' a '.jpg'")
    else:
        print("No se encontró ninguna variante que funcione. Se recomienda:")
